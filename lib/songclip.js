...
class Songclip {
   ...
   *
   * Search for clips with a query string or tags. Optional: minLength & maxLength, page and limit
   *
   ...
   * @param {number} params.allowDuplicates Allow multiple duplicates for same song (default: true)
  ...
  async search(params = {
    ...
    const { q, context = {}, page = SNGCLP_DEFAULT_PAGE, limit = SNGCLP_DEFAULT_LIMIT, minLength = 0, maxLength = 10000 , allowDuplicates = true } = params
   ...
   *
   * Get clips for given collection identified by 'term'. Optional: minLength & maxLength, page and limit
   *
   * @link https://songclip.docs.apiary.io/#reference/0/songclip-audio-files/search-songclip-audio-files
   *
   * @param {Object} params                 Parameters for call
  ...
  async getCollection(params = {}) {
    const headers = this.getHeaders()
    const { term = 'trending', context = {}, page = SNGCLP_DEFAULT_PAGE, limit = SNGCLP_DEFAULT_LIMIT, minLength = 0, maxLength = 10000 } = params
    const auxContext = this.populateContext(context)
    try {
      ...
      const response = await axios({
        ...
        method: 'get',
        headers,
        params: { context: auxContext, page, limit, minLength, maxLength },
        timeout: this.config.timeout || 0,
        responseType: 'json',
        responseEncoding: 'utf8',
        maxRedirects: 5,
  ...
  async getRelated( params = {}) {
    const headers = this.getHeaders()
    const { id, context = {}, page = SNGCLP_DEFAULT_PAGE, limit = SNGCLP_DEFAULT_LIMIT, assetType = 'songclip', minLength = 0, maxLength = 10000 } = params
    const auxContext = this.populateContext(context)
    try {
      const response = await axios({
